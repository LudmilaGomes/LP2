...
/*
etapa 1 - cada thread escreve um dicionário para sua letra (usamos 26 threads)
etapa 2 - cada thread adiciona suas palavras formadas em seu próprio dicionário
etapa 3 - a thread main espera pelas threads; quando as threads terminam, 
a thread main lê de cada arquivo e salva em um arquivo geral (dicionário)

uso de barreira devido à etapa 3 (em que a thread main aguarda as outras 
para que possa ler os arquivos criados)
*/

// cada thread escreve seu próprio dicionário
...
//    funcao_thread((void*)'Z');
    ...
    */
    ...
    //funcao main consolida o dicionario em um arquivo so a partir dos arquivos gerados
